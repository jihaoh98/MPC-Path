...
class MpcCar 
{
 private:
  ...
  // the Coefficient of quadratic programming
  ...
  Eigen::SparseMatrix<double> Cu_, lu_, uu_;  // a delta ddelta constrains
                     ...
                     const double& delta) 
  {
    ...
    Ac_(0, 2) = -v * std::sin(phi);
    Ac_(0, 3) = std::cos(phi);
    Ac_(1, 2) = v * std::cos(phi);
    Ac_(1, 3) = std::sin(phi);
    Ac_(2, 3) = std::tan(delta) / ll_;
    Ad_ = Ad_ + dt_ * Ac_;
 ...
 public:
  MpcCar(ros::NodeHandle& nh) : nh_(nh) 
  {
    ...
    for (int i = 1; i < N_; ++i) 
    {
      // set the coefficient of delta and velocity
      Qx_.coeffRef(i * n - 2, i * n - 2) = rho_;
      Qx_.coeffRef(i * n - 1, i * n - 1) = 0;
    }
    // set the coefficient of the last state
  ...
  MatrixA matrixpow(MatrixA matrix, int num)
  {
    MatrixA test;
    test.setIdentity();
